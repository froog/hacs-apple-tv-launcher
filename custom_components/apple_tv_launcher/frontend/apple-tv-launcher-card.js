...
var y = globalThis, ae = (e) => e, b = y.trustedTypes, oe = b ? b.createPolicy("lit-html", { createHTML: (e) => e }) : void 0, x = "$lit$", S = `lit$${Math.random().toFixed(9).slice(2)}$`, C = "?" + S, se = `<${C}>`, w = document, T = () => w.createComment(""), E = (e) => e === null || typeof e != "object" && typeof e != "function", ce = Array.isArray, le = (e) => ce(e) || typeof e?.[Symbol.iterator] == "function", D = "[ 	\n\f\r]", O = /<(?:(!--|\/[^a-zA-Z])|(\/?[a-zA-Z][^>\s]*)|(\/?$))/g, ue = /-->/g, de = />/g, k = RegExp(`>|${D}(?:([^\\s"'>=/]+)(${D}*=${D}*(?:[^ \t\n\f\r"'\`<>=]|("|')|))|$)`, "g"), fe = /'/g, pe = /"/g, me = /^(?:script|style|textarea|title)$/i, A = ((e) => (t, ...n) => ({
	_$litType$: e,
	strings: t,
	values: n
}))(1), j = Symbol.for("lit-noChange"), M = Symbol.for("lit-nothing"), he = /* @__PURE__ */ new WeakMap(), N = w.createTreeWalker(w, 129);
function ge(e, t) {
...
var ve = class {
	...
	_$AI(e, t = this) {
		e = F(this, e, t), E(e) ? e === M || e == null || e === "" ? (this._$AH !== M && this._$AR(), this._$AH = M) : e !== this._$AH && e !== j && this._(e) : e._$litType$ === void 0 ? e.nodeType === void 0 ? le(e) ? this.k(e) : this._(e) : this.T(e) : this.$(e);
	}
	O(e) {
...
function We(e) {
	...
	let t = e, n = t.artwork;
	return n ? typeof t.expiresAt == "number" && t.expiresAt > Date.now() && typeof n.primary == "string" && (n.fallback === null || typeof n.fallback == "string") && typeof n.unframed == "boolean" && (n.trackId === void 0 || Number.isInteger(n.trackId)) : !1;
}
async function Ge(e, t, n) {
	let r = `apple-tv-launcher-artwork-v5:${`${e}|${t}`}`;
	try {
		...
		if (e) {
			...
			if (We(i)) {
				let e = i.artwork;
				return e.trackId && (e.primary = await Y(n, e.trackId, t) ?? J(e.trackId, t)), e;
			}
			window.localStorage.removeItem(r);
		}
	} catch {}
	let i = null, a = new AbortController(), o = window.setTimeout(() => a.abort(), 1e4);
	try {
		let r = new URLSearchParams({
			bundleId: e,
			country: t
		}), o = await fetch(`https://itunes.apple.com/lookup?${r}`, { signal: a.signal });
		if (o.ok) {
			let e = (await o.json()).results?.[0], r = e?.artworkUrl512 ?? e?.artworkUrl100 ?? null;
			e?.trackId ? i = {
				primary: await Y(n, e.trackId, t) ?? J(e.trackId, t),
				fallback: r,
				unframed: !0,
				trackId: e.trackId
			} : r && (i = {
...
async function Ke(e, t, n) {
	...
	if (i) {
		let r = i.trackId ? J(i.trackId, t.artwork_country) : i.url ?? null, a = i.trackId ? await Y(n, i.trackId, t.artwork_country) : null;
		return {
			...e,
			artwork: a ?? r,
			artworkFallback: null,
...
function qe(e) {
	...
	let t = String(e.artwork_country ?? X.artwork_country).trim().toLowerCase();
	return {
		entity: String(e.entity),
		...e.title === void 0 ? {} : { title: String(e.title) },
		columns: Z(e.columns, 2, 10, X.columns),
		mobile_columns: Z(e.mobile_columns, 2, 6, X.mobile_columns),
		show_labels: $(e.show_labels, X.show_labels),
		artwork_lookup: $(e.artwork_lookup, X.artwork_lookup),
		artwork_country: /^[a-z]{2}$/.test(t) ? t : X.artwork_country,
		wake_before_launch: $(e.wake_before_launch, X.wake_before_launch),
		wake_delay: Z(e.wake_delay, 0, 15e3, X.wake_delay),
		retry: $(e.retry, X.retry),
		app_order: Q(e.app_order),
		include: Q(e.include),
		exclude: Q(e.exclude),
...
function Ye(e, t) {
	...
	}), i = (t.include.length ? r.filter((e) => Je(e, t.include)) : r).filter((e) => !Je(e, t.exclude)), a = new Map(t.app_order.map((e, t) => [e.toLocaleLowerCase(), t])), o = (e) => a.get(e.name.toLocaleLowerCase()) ?? (e.id ? a.get(e.id.toLocaleLowerCase()) : void 0) ?? 2 ** 53 - 1;
...
`, $e = "apple-tv-launcher-card", et = new URL(
	/* @vite-ignore */
	"./fallback.png",
	import.meta.url
).href;
function tt() {
	...
	return typeof e == "string" && /^[A-Za-z]{2}$/.test(e) ? e.toLowerCase() : X.artwork_country;
}
var nt = "0.1.0", rt = (e) => new Promise((t) => window.setTimeout(t, e));
function it(e, t) {
	return {
		name: e,
		id: t,
...
var ot = class extends z {
	...
	static getStubConfig() {
		return {
			...
			columns: X.columns,
			mobile_columns: X.mobile_columns
		};
	}
	...
	setConfig(e) {
		let t = qe(e), n = this._discoveryConfigSignature;
	...
	getCardSize() {
		let e = this._config?.columns ?? X.columns;
	...
	async _loadApps(e) {
		...
		r = Ye(r, n), this._apps = r, this._loading = !1, this.requestUpdate();
	...
	render() {
		let e = this._config;
		if (!e) return M;
		let t = this._entityState, n = Xe(t), r = e.title ?? t?.attributes.friendly_name ?? "Apple TV", i = t?.attributes.app_name ?? t?.attributes.source ?? "";
		return A`
      <ha-card style=${q({
	...
	_renderContents(e) {
		if (this._loading) {
			let e = Math.max(this._config?.columns ?? X.columns, 5);
	...
	_renderApp(e, t) {
		let n = t.toLocaleLowerCase() === e.name.toLocaleLowerCase(), r = this._launching === e.name;
		return A`
      <button
        class=${U({
			app: !0,
      ...
      >
        <span
          ...
          style=${q({ "--tile-bg": `url(${et}) center / cover no-repeat` })}
        >
          <span class="fallback">${Ze(e.name)}</span>
          ${e.artwork ? A`<img
                ...
                />` : M}
          ${r ? A`<span class="spinner" aria-hidden="true"></span>` : M}
        </span>
...
customElements.get("apple-tv-launcher-card") || customElements.define($e, ot), window.customCards = window.customCards ?? [], window.customCards.some((e) => e.type === "apple-tv-launcher-card") || window.customCards.push({
	type: $e,
...
//#endregion
export { ot as AppleTvLauncherCard, $e as CARD_TAG, nt as CARD_VERSION };
